...
#include <vector>
#include "ItemType.h"
#include "Loaders/DatAttributes.h"

void ItemType::unserialize(uint16_t cid, std::ifstream& fin) {
    clientId = cid;
...
//                m_attribs.set(attr, light);
    ...
    if(!done)
        throw std::runtime_error("Couldn't finish unserializing attributes in: " + clientId);
    ...
    // Skip sprite IDs
    uint32_t spriteCount = width * height * layers * patternX * patternY * patternZ * animationPhases;
